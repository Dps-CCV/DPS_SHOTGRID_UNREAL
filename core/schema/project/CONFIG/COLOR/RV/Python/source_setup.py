    ...
    def sourceSetup(self, event, noColorChanges=False):
        ...
        if len(srcData) > 0:
            ...
            for key in dataMap.keys():
                try:
                    exec("%s = %s" % (dataMap[key], dataDict[key]))
                except KeyError:
                    pass
        ...
        if ext == "DPX":
            if DPXCreator == "AppleComputers.libcineon" or DPXCreator == "AUTODESK":
                #
                ...
                if float(PixelAspectRatio) == 0.0:
                    self.setPixelAspect(lensNode, 1.0)
            elif (ColorSpace == "" or ColorSpace == "Other (0)") and (
            ...
            TransferFunction == "" or TransferFunction == "ARRI LogC"
        ):
            #
        ...
        elif ext == "ARI" and TransferFunction == "ARRI LogC Film":
            #
            #  Assume tif files are linear if there's no other indication
            #

            TransferFunction = "ALEXA LogC Film"
            ...
            ext in ["J2C", "J2K", "JPT", "JP2"] and ColorSpacePrimaries == "UNSPECIFIED"
        ):
            #
        ...
        if ICCProfileDesc != "":
            if ICCNode and len(ICCProfileData) > 0:
                commands.setStringProperty(
                    ICCNode + ".inProfile.description", [ICCProfileDesc], True
                )
                commands.setFloatProperty(
                    ICCNode + ".inProfile.version", [float(ICCProfileVer)], True
                )
                commands.setByteProperty(
                    ICCNode + ".inProfile.data", ICCProfileData, True
                )
        ...
        if not noColorChanges:
            ...
            #
            TransferFunction = self.checkEnvVar(
                ext, mInfo["bitsPerChannel"], TransferFunction
            )
