...
class TkArnoldNodeHandler(object):
    ...
    TK_EXTRA_PLANE_TEMPLATE_MAPPING = {
        "sgtk_ar_aov_separate_file#": "output_aov_render_template"
    }
    ...
    def reset_render_path(self, node=None):
        ...
        publish_template_parm.set(self.get_publish_template(node, ass=ass))
    ...
    def get_publish_template(self, node=None, ass=False):
        output_profile = self._get_output_profile(node)
        if ass == True:
            if node.parm('trange').evalAsInt() == 0:
                publish_cache_template = output_profile["output_publish_ass"]
            else:
                publish_cache_template = output_profile["output_publish_seq_ass"]
        else:
            publish_cache_template = output_profile["output_publish_render"]
    ...
    def _compute_and_set(self, node, parm_name, template_name, aov_name=None):
        ...
        """
        if node.parm("trange").evalAsInt()!=0 and template_name == "output_ass_template":
            template_name = "output_ass_seq_template"
        try:
